...
- `--verify-vision`: (선택·스트레치) 04-WIREFRAME.html을 렌더→이미지로 캡처해 §5.4.1 상태·스펙과 대조 검증 (Phase 4). 렌더 도구가 없으면 자동 스킵
...
### Phase 4: Frontend Review (Quality Gate)
...
- 04-WIREFRAME.html을 렌더→이미지로 캡처하여 §5.4.1 상태 매트릭스·03-SCREEN-SPEC.md와 대조 검증 (Opus 4.8 vision + self-verify).
- 확인: 체크된 상태(빈/로딩/에러/성공)가 실제 화면에 표현됐는가 · 페이지·컴포넌트 누락 · 반응형 분기 반영 여부.
- 비차단·스트레치: 렌더 도구(headless 브라우저 등)가 없으면 건너뛰고 텍스트 리뷰 결과만 사용.
...
4. **State Coverage**: §5.4.1에 체크된 모든 상태에 마이크로카피 또는 UI 처리를 명세합니다
